...
! Velocity boundary conditions: no-slip stationary walls, top plate imposed with uniform velocity
! Pressure boundary condition: Neumann boundary condition at all walls (requires a reference pressure value)
! -------------------------------------------------------------------------------------------------------------------------------------------------
! The pressure correction loop is solved using Jacobi iterative method (parallelized using OpenMP)
! The diffusion fluxes are discretized using linear-profile assumption
! The convective fluxes are discretized using a hybrid scheme of 1st order upwind and central differencing scheme
! -------------------------------------------------------------------------------------------------------------------------------------------------
! To compile and run the program, use the following command at the terminal:
! > gfortran -fopenmp file_name.f90 -o executable_name.out
! > ./executable_name.out
! The flag -fopenmp can be switched off to run the code in serial mode
...
MODULE SOLVER_MEMORY
  ...
  DOUBLE PRECISION   :: rho, mu, Re, lid_velocity, beta
  DOUBLE PRECISION   :: alpha_u, alpha_v, alpha_p, mass_error

  DOUBLE PRECISION, DIMENSION(:, :), ALLOCATABLE :: u, v, p, u_old, v_old, pprime, pprime_old
  ...
  DOUBLE PRECISION, DIMENSION(:, :), ALLOCATABLE :: upw, cds
...
CONTAINS
  ...
  SUBROUTINE ALLOCATE_MEMORY()
    ...
    ALLOCATE (pprime_old(0:nx+1, 0:ny+1)                                            )
    ALLOCATE (upw(0:nx+1, 0:ny+1), cds(0:nx+1, 0:ny+1)                  )
  ...
  SUBROUTINE SET_PARAMETERS()
    ...
    nx = 129                    ! set number of points in x-direction
    ny = 129                    ! set number of points in y-direction
    ...
    Re           = 1000.0d0                                  ! set Reynolds number
    rho          = 1.0d0                                     ! set density of fluid
    lid_velocity = 1.0d0                                     ! set top plate velocity
    mu           = (rho * lid_velocity * (xmax - xmin)) / Re ! calculating dynamic viscosity
  ...
  SUBROUTINE U_MOMENTUM_SOLVER()
    ...
    DO j = 1, ny
       DO i = 2, nx
          ...
          upw(i, j)  = ((MAX(-me, 0.0d0) + MAX(mw, 0.0d0) + MAX(-mn, 0.0d0) +       &
               &        MAX(ms, 0.0d0)) * u(i, j)) - (MAX(-me, 0.0d0) * u(i+1, j) + &
               &        MAX(mw, 0.0d0)  * u(i-1, j) + MAX(-mn, 0.0d0) * u(i, j+1) + &
               &        MAX(ms, 0.0d0)  * u(i, j-1))

          cds(i, j)  = 0.50d0 * (me * (u(i, j) + u(i+1, j)) - &
               &                 mw * (u(i, j) + u(i-1, j)) + &
               &                 mn * (u(i, j) + u(i, j+1)) - &
               &                 ms * (u(i, j) + u(i, j-1)))
  ...
  SUBROUTINE V_MOMENTUM_SOLVER()
    ...
    DO j = 2, ny
       DO i = 1, nx
          ...
          upw(i, j)  = ((MAX(-me, 0.0d0) + MAX(mw, 0.0d0) + MAX(-mn, 0.0d0) +       &
               &        MAX(ms, 0.0d0)) * v(i, j)) - (MAX(-me, 0.0d0) * v(i+1, j) + &
               &        MAX(mw, 0.0d0)  * v(i-1, j) + MAX(-mn, 0.0d0) * v(i, j+1) + &
               &        MAX(ms, 0.0d0)  * v(i, j-1))

          cds(i, j)  = 0.50d0 * (me * (v(i, j) + v(i+1, j)) - &
               &                 mw * (v(i, j) + v(i-1, j)) + &
               &                 mn * (v(i, j) + v(i, j+1)) - &
               &                 ms * (v(i, j) + v(i, j-1)))
  ...
  SUBROUTINE PRESSURE_CORRECTION_SOLVER()
    ...
    ! solving for pressure correction equation; parallelize it using OpenMP --> still work to do
    DO k = 1, pressure_iter_max
       !$OMP PARALLEL DO NUM_THREADS(NUM_THREAD) COLLAPSE(2) DEFAULT(shared) PRIVATE(i, j) 
       DO j = 1, ny
          DO i = 1, nx
             
             pprime(i, j) = (1.0d0 / ap_p(i, j)) * &
                  &         (ae(i, j) * pprime_old(i+1, j) + &
                  &          aw(i, j) * pprime_old(i-1, j) + &
                  &          an(i, j) * pprime_old(i, j+1) + &
                  &          as(i, j) * pprime_old(i, j-1) - &
                  &          mass_imb(i, j))             
...
PROGRAM MAIN
  ...
  WRITE(*, '(A, F15.8)') "Governing parameter: U_plate = ", lid_velocity
